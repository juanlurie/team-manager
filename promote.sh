...
if ! docker image inspect tm-dev-api:latest &>/dev/null || ! docker image inspect team-manager-ui:dev &>/dev/null; then
  echo "✗ Dev images not found. Run './dev.sh build' first."
  exit 1
fi

echo "  Tagging images..."
docker tag tm-dev-api:latest team-manager-api:prod
