...
import { CreateMeetingSeriesSlotRequest } from '../../core/models/meeting-series.model';
  ...
  template: `
    <div class="page">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px">
        ...
        <h2 style="margin:0;font-size:1.3rem;font-weight:700">Create Meeting Series</h2>
      </div>

      <div class="section">
        <mat-form-field appearance="fill" class="full-width">
          <mat-label>Series Title</mat-label>
          <input matInput [ngModel]="title()" (ngModelChange)="title.set($event)" maxlength="200" placeholder="e.g. Performance Reviews Q2 2026">
        </mat-form-field>
        <mat-form-field appearance="fill" class="full-width">
...
export class MeetingSeriesCreateComponent implements OnInit {
  ...
  goBack() { this.router.navigate(['/meeting-series']); }
  ...
  save() {
    ...
    if (!this.title().trim()) { this.error.set('Enter a series title'); return; }
    ...
    this.svc.create({
      ...
      next: () => {
        this.snack.open('Series created!', 'OK', { duration: 2000 });
        this.router.navigate(['/meeting-series']);
      },
      error: () => this.error.set('Failed to create series')
    });
  }
}