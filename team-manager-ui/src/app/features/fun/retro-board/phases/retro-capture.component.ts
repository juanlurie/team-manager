...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RespondedMeterComponent, NoteCommentsComponent],
      ...
      <div class="cols">
        @for (c of s.columns; track c.id) {
          <div class="col" [style.borderColor]="c.color+'55'">
            ...
            <!-- Capture is over once the facilitator moves on, so the composer goes rather than
                 firing a request the server will reject. See the store's canAddNotes. -->
            @if (store.canAddNotes()) {
              ...
              <p class="muted" style="margin:8px 0 12px;font-size:13px">{{ store.captureClosedHint() }}</p>
            }
