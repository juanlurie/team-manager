...
[ApiController]
[RequireFeature("2048")]
[Route("api/v1/game-2048")]
[Authorize]
public class Game2048Controller(Game2048Service service) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    [RequireFeature("2048-host")]
    public async Task<IActionResult> CreateSession([FromBody] CreateGame2048SessionRequest request)
    {
    ...
    public async Task<IActionResult> MakeMove(Guid id, [FromBody] Game2048MoveRequest request)
    {
        var memberId = GetCurrentMemberId();
        if (!memberId.HasValue) return Unauthorized();
