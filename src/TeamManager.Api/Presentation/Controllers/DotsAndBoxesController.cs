...
[ApiController]
[RequireFeature("dots-and-boxes")]
[Route("api/v1/dots-and-boxes")]
[Authorize]
public class DotsAndBoxesController(DotsAndBoxesService service) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    [RequireFeature("dots-and-boxes-host")]
    public async Task<IActionResult> CreateSession([FromBody] CreateDotsAndBoxesSessionRequest request)
    {
    ...
    public async Task<IActionResult> MakeMove(Guid id, [FromBody] MakeDotsAndBoxesMoveRequest request)
    {
        var memberId = GetCurrentMemberId();
        if (!memberId.HasValue) return Unauthorized();
        var (session, error) = await service.MakeMoveAsync(id, memberId.Value, request);
        if (session is null && error is null) return NotFound();
        if (error is not null) return Conflict(new { error });
        return Ok(session);
    }
